...
export function SistemaScreen() {
  ...
  const cafWarn = nowH >= 14
    ? { cls: 'text-[#f87171] border-[rgba(248,113,113,0.3)] bg-[rgba(248,113,113,0.08)]', ico: AlertCircle, txt: 'Pasadas las 14h. La cafeína puede romper el sueño.' }
    : caffeine > 0 && inMedWindow
      ? { cls: 'text-[#f87171] border-[rgba(248,113,113,0.3)] bg-[rgba(248,113,113,0.08)]', ico: AlertCircle, txt: 'Cerca de tu medicación. Puede subir ansiedad.' }
      : { cls: 'text-[#00D4AA] border-[rgba(0,212,170,0.25)] bg-[rgba(0,212,170,0.06)]', ico: CheckCircle, txt: 'Cafeína en ventana segura.' };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pb-6 space-y-3.5 min-h-dvh">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#94a0b8] uppercase tracking-wider">Híbrido 80/20</p>
          </div>
        </div>
      </motion.div>
      ...
      <Card variant="glass" className="p-3 border-[rgba(0,212,170,0.25)]">
        <div className="flex gap-2.5 items-start">
          ...
          <p className="text-[11.5px] leading-relaxed text-[#94a0b8]">
            El <b className="text-[#00D4AA]">algoritmo</b> decide y calcula. La <b className="text-[#a78bfa]">LLM on-device</b> solo rellena plantillas. <b className="text-[#34d399]">Zod</b> bloquea alucinaciones antes de la UI.
          </p>
        </div>
      </Card>
      ...
      <div>
        <div className="flex items-center gap-2 mb-2 mt-2">
          <Users size={14} className="text-[#94a0b8]" />
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#94a0b8]">Agentes</h2>
          <span className="text-[10px] text-[#94a0b8] font-mono ml-auto">4 activos</span>
        </div>
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <div>
                  ...
                  <div className="text-[9px] text-[#94a0b8] uppercase tracking-wider">{a.role}</div>
                </div>
              </div>
              <div className="text-[10px] text-[#94a0b8] font-mono">{a.status === 'busy' ? 'check-ins (exercise)' : 'esperando input'}</div>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <Card className="p-3">
        <div className="flex items-center gap-2 mb-2">
          <BatteryCharging size={15} className="text-[#94a0b8]" />
          <span className="text-sm font-bold flex-1">Fatiga de decisión</span>
        ...
        <div className="mt-2.5 space-y-1.5">
          {[
            { label: 'Hora del día', value: hourClamped, color: '#4CC9F0', weight: '35%', icon: Clock },
            { label: 'Entrenamientos hoy', value: workoutFatigue, color: '#FF6B35', weight: '25%', icon: Dumbbell },
            { label: 'Decisiones tomadas', value: interactionFatigue, color: '#a78bfa', weight: '40%', icon: Brain },
          ].map((item) => {
            const Icon = item.icon;
            return (
              <div key={item.label} className="flex items-center gap-2">
                <Tooltip content={item.label} side="top" align="center">
                  <Icon size={11} className="text-[#94a0b8] shrink-0" />
                </Tooltip>
                ...
                <span className="text-[8px] text-[#5c6577] font-mono tabular-nums w-[28px] text-right">{item.value}%</span>
              </div>
            );
          })}
        </div>
        <p className="text-[10px] text-[#94a0b8] font-mono mt-2">
          {combinedFatigue > 70
            ? 'Demasiadas elecciones. Activa Autopilot.'
            : combinedFatigue > 40
              ? 'Subiendo. Considera Autopilot.'
              : 'Baja. Puedes elegir con claridad.'}
        </p>
      </Card>
      ...
      <Card className="p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-bold flex-1">Cafeína × medicación</span>
          <span className="text-sm font-black font-mono text-[#fbbf24]">{caffeine} mg</span>
        </div>
        <div className="flex gap-2 mb-2">
          <button onClick={() => { setCaffeine(c => c + 60); trackDecision(2); }} className="flex-1 rounded-xl border border-white/[.07] bg-white/[.04] p-2 text-[11px] font-semibold text-[#94a0b8] hover:bg-white/[.08] transition-colors">
            <Coffee size={14} className="mx-auto mb-0.5" /> Espresso <span className="block text-[9px] text-[#5C6577]">+60mg</span>
          </button>
          <button onClick={() => { setCaffeine(c => c + 80); trackDecision(2); }} className="flex-1 rounded-xl border border-white/[.07] bg-white/[.04] p-2 text-[11px] font-semibold text-[#94a0b8] hover:bg-white/[.08] transition-colors">
            <Coffee size={14} className="mx-auto mb-0.5" /> Latte <span className="block text-[9px] text-[#5C6577]">+80mg</span>
          </button>
          <button onClick={() => { setCaffeine(0); trackDecision(2); }} className="flex-1 rounded-xl border border-white/[.07] bg-white/[.04] p-2 text-[11px] font-semibold text-[#94a0b8] hover:bg-white/[.08] transition-colors">
            <BoltIcon size={14} className="mx-auto mb-0.5" /> Reset
          </button>
        </div>
      ...
      <Card className="p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-bold flex-1">Horas low-stim del gym</span>
        ...
        <p className="text-[10px] text-[#94a0b8] font-mono mt-2">Gym Anxiety Mode: entrena en franjas verdes.</p>
      </Card>

      {/* Sensory Profile */}
      <div>
        <div className="flex items-center gap-2 mb-2 mt-2">
          <MoonStar size={14} className="text-[#94a0b8]" />
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#94a0b8]">Perfil sensorial</h2>
          <span className="text-[10px] text-[#94a0b8] font-mono ml-auto">neurodivergente</span>
        </div>
        <div className="space-y-2">
          {[
            { key: 'quiet', icon: VolumeX, label: 'Modo silencio', desc: 'Sin sonidos, solo hápticos' },
            { key: 'dim', icon: MoonStar, label: 'Modo dim', desc: 'Reduce brillo y contraste' },
            { key: 'swap', icon: ArrowLeftRight, label: 'Sensory swap', desc: 'Reemplaza ejercicios de alto impacto' },
          ].map((tog, i) => {
              ...
              >
                <div>
                  <div className="text-sm font-semibold flex items-center gap-2">
                    <tog.icon size={15} className="text-[#94a0b8]" /> {tog.label}
                  </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-2 mt-4">
          <Box size={14} className="text-[#94a0b8]" />
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#94a0b8]">Stack de producción</h2>
        </div>
        <div className="space-y-1.5">
          {STACK_LAYERS.map((layer, i) => (
            ...
            >
              ...
              <div className="text-[10px] text-[#94a0b8] font-mono mt-0.5">{layer.desc}</div>
            </motion.div>
          ))}
        </div>
      </div>
